...
__device__ Complex complexScalarMult(Complex a, Complex b, int scalar)
{
...
__global__ void complexProcess(Complex *a, Complex *b, Complex *c, int size, int scalar)
{
    ...
    c[tid] = complexScalarMult(a[tid], b[tid], scalar);
}
...
__host__ Complex *transformFromSignalToTimeDomain(cufftHandle plan, int SIZE, cufftComplex *d_c)
{
    ...
    checkCudaErrors(cudaMemcpy(results, d_c, SIZE * sizeof(Complex), cudaMemcpyDeviceToHost));
