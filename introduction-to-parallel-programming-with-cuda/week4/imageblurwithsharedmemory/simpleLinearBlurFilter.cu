...
__global__ void applySimpleLinearBlurFilter(uchar *r, uchar *g, uchar *b, int rows, int columns, int tileWidth)
{
    ...
    __shared__ int sharedMemR[TILEWIDTH][TILEWIDTH + 2];
    __shared__ int sharedMemG[TILEWIDTH][TILEWIDTH + 2];
    __shared__ int sharedMemB[TILEWIDTH][TILEWIDTH + 2];
    ...
    if(y < rows && x < columns)
    {
        sharedMemR[threadIdx.y][base + threadIdx.x] = r[y * columns + x];
        sharedMemG[threadIdx.y][base + threadIdx.x] = g[y * columns + x];
        sharedMemB[threadIdx.y][base + threadIdx.x] = b[y * columns + x];

        if(threadIdx.x == 0 && x != 0)
        {
            sharedMemR[threadIdx.y][base + threadIdx.x - 1] = r[y * columns + x - 1];
            sharedMemG[threadIdx.y][base + threadIdx.x - 1] = g[y * columns + x - 1];
            sharedMemB[threadIdx.y][base + threadIdx.x - 1] = b[y * columns + x - 1];
        }
        else if(threadIdx.x == 0  && x == 0)
        {
            sharedMemR[threadIdx.y][base + threadIdx.x - 1] = 0;
            sharedMemG[threadIdx.y][base + threadIdx.x - 1] = 0;
            sharedMemB[threadIdx.y][base + threadIdx.x - 1] = 0;           
        }

        if(threadIdx.x == tileWidth - 1 && x != columns - 1)
        {
            sharedMemR[threadIdx.y][base + threadIdx.x + 1] = r[y * columns + x + 1];
            sharedMemG[threadIdx.y][base + threadIdx.x + 1] = g[y * columns + x + 1];
            sharedMemB[threadIdx.y][base + threadIdx.x + 1] = b[y * columns + x + 1];
    ...
    if(y < rows && x < columns)
    {
        r[y * columns + x] = (sharedMemR[threadIdx.y][base + threadIdx.x - 1] + sharedMemR[threadIdx.y][base + threadIdx.x] + sharedMemR[threadIdx.y][base + threadIdx.x + 1])/3;
        g[y * columns + x] = (sharedMemG[threadIdx.y][base + threadIdx.x - 1] + sharedMemG[threadIdx.y][base + threadIdx.x] + sharedMemG[threadIdx.y][base + threadIdx.x + 1])/3;
        b[y * columns + x] = (sharedMemB[threadIdx.y][base + threadIdx.x - 1] + sharedMemB[threadIdx.y][base + threadIdx.x] + sharedMemB[threadIdx.y][base + threadIdx.x + 1])/3;
    }
}
...
__host__ void executeKernel(uchar *r, uchar *g, uchar *b, int rows, int columns, int tileWidth)
{
    cout << "Executing kernel\n";
    //Launch the convert CUDA Kernel
    dim3 threadsPerBlock(tileWidth, tileWidth, 1);
    dim3 grid((rows + tileWidth - 1)/tileWidth, (columns + tileWidth - 1)/tileWidth, 1);

    applySimpleLinearBlurFilter<<<grid, threadsPerBlock>>>(r, g, b, rows, columns, tileWidth);
...
__host__ std::tuple<std::string, std::string, std::string, int> parseCommandLineArguments(int argc, char *argv[])
{
    ...
    cout << "inputImage: " << inputImage << " outputImage: " << outputImage << " currentPartId: " << currentPartId << " threadsPerBlock: " << tileWidth << "\n";
    return {inputImage, outputImage, currentPartId, tileWidth};
}
...
int main(int argc, char *argv[])
{
    ...
    int tileWidth = get<3>(parsedCommandLineArgsTuple);
    try 
    {
        auto[rows, columns, r, g, b] = readImageFromFile(inputImage);

        executeKernel(r, g, b, rows, columns, tileWidth);
