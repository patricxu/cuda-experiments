...
__host__ float * runStreamsFullAsync(float *host_mem, int num_elements)
{
    ...
    copyFromHostToDeviceSync(host_mem, dev_mem, num_elements);
    ...
    kernelA1<<<blocksPerGrid,threadsPerBlock,0, stream1>>>(dev_mem, num_elements, x);
    ...
    kernelB2<<<blocksPerGrid,threadsPerBlock,0, stream4>>>(dev_mem, num_elements);


    // Copy device memory back to host asynchronously

    copyFromDeviceToHostAsync(dev_mem, host_mem, num_elements, stream1);
